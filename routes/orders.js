    ...
    if(err) throw err;
      ...
      con.query(sql, [sess.info.member_id], (err,result)=>{
        if(err){
          ...
          var sql = "select o.order_no, date_format(o.order_date,'%y-%m-%d %T') as order_date, o.order_state ,date_format(o.order_rec,'%y-%m-%d %T') as order_rec,p.prod_id, p.prod_img, p.prod_name, op.orders_prod_count, op.orders_prod_price, op.entrep_id from prod p, orders_prod op, orders o where p.prod_id = op.prod_id and o.order_state='수령' and op.order_no = o.order_no and op.entrep_id=p.entrep_id and o.member_id = ? ORDER BY order_no ASC";
          con.query(sql, [sess.info.member_id], (err,row)=>{
            if(err){
              ...
              res.render('index', { page: './sub/orderList', sess : sess, price : price, order_no : order_no, img : img, name : name, state : state, result : result, date: date, price2 : price2, order_no2 : order_no2, img2 : img2, name2 : name2, state2 : state2, data2 : row, date2: date2, entrep2:entrep2, rec2: rec2});
            }
          });
        }
      });
...
router.post('/cartOrder',function(req,res,next){
  ...
  pool.getConnection((err,conn)=>{
    ...
    conn.query(sql,[sess.info.member_id],(err,row)=>{
      ...
      var sql="insert into orders values(null,?,'대기',?,now(),?,?,0,?)";
...
router.post("/orders/:p_id/:e_id",function(req,res,next){
  var sess=req.session;
  var p_id=req.params.p_id;
  var e_id=req.params.e_id;
  pool.getConnection(function(err,conn){
    ...
    var sql="insert into orders values(null,?,'대기',?,now(),?,?,0,?)"
    conn.query(sql, [sess.info.member_id,req.body.o_price,req.body.card_no,req.body.order_hope_rec,req.body.o_tel],(err,row)=>{
      if(err){
        throw err;
      }
      if(row){
        var sql="select prod.entrep_id from prod where prod_id=? and entrep_id=?"
        conn.query(sql,[p_id,e_id],(err,row)=>{
          if(err){
            throw err;
          }
