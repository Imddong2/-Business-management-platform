...
router.get('/e_prod', function (req, res, next) {
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      var sql_prod = "select p.prod_id, p.prod_img, p.prod_name, p.prod_state, pc.prod_class_id, p.prod_price from prod_class pc, prod p where p.entrep_id = ? and pc.prod_class_id = p.prod_class_id";
      ...
      con.query(sql_order, [sess.info.entrep_id], (err, data) => {
        ...
        if (err) {
          ...
          for(var j = 0; j < order_no.length; j++){
            for(var k = j; k <= order_no.length; k++){
              console.log(order_no[j], order_no[j+1]);
...
router.get('/e_order', function(req, res, next){
  var sess = req.session;
  var order_no = [];
  var img = [];  var name = [];  var date = [];  var state = [];  var price = []; var rec = [];
  pool.getConnection(function(err,con){
    if(err) throw err;
    else{
      var sql = "select o.order_no, date_format(o.order_date,'%y-%m-%d %H:%i') as order_date, date_format(o.order_hope_rec,'%y-%m-%d %H:%i') as order_hope_rec, o.order_state ,p.prod_id, p.prod_img, p.prod_name, op.orders_prod_count, op.orders_prod_price from prod p, orders_prod op, orders o where p.prod_id = op.prod_id and op.order_no = o.order_no and p.entrep_id = ? and op.entrep_id = p.entrep_id ORDER BY order_no ASC";
      con.query(sql, [sess.info.entrep_id], (err,result)=>{
        ...
        if(err){
          ...
          for(var i = 0; i < result.length; i++){
            ...
            rec.push(result[i].order_hope_rec);
          ...
          res.render('index', { page: './sub/e_order', sess : sess,rec : rec, price : price, order_no : order_no, img : img, name : name, state : state, result : result, date: date});
        }
      });
    }
  });
...
router.get('/e_regProd', function(req, res, next){
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      var prod_class = "select prod_class_id from prod_class";
      conn.query(prod_class,[], (err, prod_class)=>{
        conn.release();
        if(err) throw err;
        else{
          console.log(prod_class.prod_class_id);
          res.render('index', {page : './sub/e_regProd', sess : sess, prod_class: prod_class });
        }
      });
    }
  });
          ...
          conn.query(sql_stat_prod, [sess.info.entrep_id], (err, stat_prod) => {
            ...
            if(err){
              ...
              for(var l = 0; l < stat_prod.length; l++){
                for(var m = l; m < stat_prod.length; m++){
          ...
          conn.query(sql_stat_prod_price, [sess.info.entrep_id,sess.info.entrep_id], (err, stat_price) => {
            ...
            if(err){
              ...
              for(var l = 0; l < stat_price.length; l++){
                for(var m = l; m < stat_price.length; m++){
                  if(prod_name[l] == prod_name[l+1]){
                    // console.log(prod_name[j]);
    ...
    if(err){
      ...
      pool.getConnection(function(err, conn){
        if(err){
          ...
          var sql_stat_prod = "select DISTINCT o.order_no, o.member_id, m.member_name, o.order_price from members m, orders o, orders_prod op where m.member_id = o.member_id and o.order_no = op.order_no and op.entrep_id = ? and o.order_date >= date_format(now(), '%Y-%m-01') and o.order_date<= last_day(now()) and o.order_state = '수령'";
          // var sql_stat_prod = "select p.prod_name, op.orders_prod_count, op.orders_prod_price from orders o, orders_prod op, prod p where p.prod_id = op.prod_id and op.order_no = o.order_no and p.entrep_id = ? and o.order_date between ? and ?";
          conn.query(sql_stat_prod, [sess.info.entrep_id], (err, stat_member_prod) => {
            ...
            if(err){
              ...
              for(var l = 0; l < stat_member_prod.length; l++){
                for(var m = l; m < stat_member_prod.length; m++){
    ...
    if(err){
      ...
      pool.getConnection(function(err, conn){
        if(err){
          ...
          var sql_stat_prod_price = "select DISTINCT o.order_no, o.member_id, m.member_name, o.order_price from members m, orders o, orders_prod op where m.member_id = o.member_id and o.order_no = op.order_no and op.entrep_id = ? and o.order_date >= date_format(now(), '%Y-%m-01') and o.order_date<= last_day(now()) and o.order_state = '수령'";
          ...
          conn.query(sql_stat_prod_price, [sess.info.entrep_id], (err, stat_prod_price) => {
            ...
            if(err){
              ...
              for(var l = 0; l < stat_prod_price.length; l++){
                for(var m = l; m < stat_prod_price.length; m++){
                  if(member_name[l] == member_name[l+1]){
                    // console.log(prod_name[j]);
...
router.post('/e_regProd', upload.array('photo'), function(req, res, next){
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      for(var i = 1; i < 4; i++){
        if(req.files[i]){
          detail[i] = 'images/' + req.files[i].originalname;
        }
      }
      console.log(detail);
          ...
          var sql_updat_img = "update prod set prod_price = ?, prod_name =?, prod_state =?, entrep_id=?, prod_img=?, prod_class_id=? where prod_id = ? and entrep_id = ?";
            conn.query(sql_updat_img, [req.body.price, req.body.name, req.body.state, sess.info.entrep_id, imgurl_thum, req.body.class_id, prod_id[1], sess.info.entrep_id], (err, update_img) =>{
              if(err){
                ...
                res.writeHead(200, { "content-type": "text/html; charset=utf-8" });
                res.write("<script>alert('수정완료.');location.href='/entrep/e_prod';</script>");
              }
            });
          }
      });
...
router.post('/e_entrep', upload.single('photo'), function(req, res, next){
  var sess = req.session;
  var imgurl_logo = 'images/';
  console.log(req.file);

  if(req.file){
    imgurl_logo = 'images/' + req.file.originalname;
  }else{
    imgurl_logo = sess.info.entrep_logo;
  }
        ...
        if(err){
          ...
          conn.query(sql_session, [sess.info.entrep_id, sess.info.entrep_pwd], (err,session) =>{
            if(err){
              ...
              sess.info = session[0];
              res.send(`<script> location.href="/entrep/e_main" </script>`);
...
router.post('/e_stat_prod_sum', function(req, res, next){
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      var sql_stat_prod = "select DISTINCT o.order_no, p.prod_name, op.orders_prod_count, op.orders_prod_price from orders o, orders_prod op, prod p where p.prod_id = op.prod_id and op.order_no = o.order_no and p.entrep_id = ? and o.order_date >= date_format(?, '%Y-%m-%d') and o.order_date<= date_format(?, '%Y-%m-%d') and order_state = '수령'";
      // var sql_stat_prod = "select p.prod_name, op.orders_prod_count, op.orders_prod_price from orders o, orders_prod op, prod p where p.prod_id = op.prod_id and op.order_no = o.order_no and p.entrep_id = ? and o.order_date between ? and ?";
...
router.post('/e_stat_prod_price', function(req, res, next){
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      var sql_stat_prod_price = "select DISTINCT o.order_no, op.orders_prod_count, op.orders_prod_price, p.prod_name From orders_prod op, orders o, prod p where o.order_no = op.order_no and op.entrep_id = ? and o.order_date >= date_format(?, '%Y-%m-%d') and o.order_date <= date_format(?, '%Y-%m-%d') and o.order_state = '수령'";
...
router.post('/e_stat_member_sum', function(req, res, next){
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      var sql_stat_prod = "select DISTINCT o.order_no, o.member_id, m.member_name, o.order_price from members m, orders o, orders_prod op where m.member_id = o.member_id and entrep_id = ? and o.order_date >= date_format(?, '%Y-%m-%d') and o.order_date<= date_format(?, '%Y-%m-%d') and o.order_state ='수령'";
      // var sql_stat_prod = "select p.prod_name, op.orders_prod_count, op.orders_prod_price from orders o, orders_prod op, prod p where p.prod_id = op.prod_id and op.order_no = o.order_no and p.entrep_id = ? and o.order_date between ? and ?";
...
router.post('/e_stat_member_price', function(req, res, next){
  ...
  pool.getConnection((err, conn) => {
    if(err){
      ...
      var sql_stat_prod_price = "select DISTINCT o.order_price, m.member_name, o.order_no from orders o, members m, orders_prod op where o.member_id = m.member_id and o.order_date >= date_format(?, '%Y-%m-%d') and o.order_date<= date_format(?, '%Y-%m-%d') and op.entrep_id = ? and o.order_state = '수령'";
    ...
    if(err){
      ...
      conn.query(sql_submit, [order_id[1]], (err, submit) => {
        ...
        if(err){
          ...
          res.writeHead(200, { "content-type": "text/html; charset=utf-8" });
          res.write("<script>alert('등록이 완료되었습니다.');location.href='/entrep/e_order';</script>");
        }
      });
    }
  });
