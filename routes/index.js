...
router.get('/prodDetail/:p_id/:e_id', function (req, res, next) {
  ...
  pool.getConnection((err, conn) => {
    ...
    conn.query(sql, [p_id, e_id], function (err, row) {
      ...
      if (row) {
        var sql = "select detail_img_name from detail_img where prod_id=? and entrep_id=? and detail_img_name!='images/'";
...
router.get('/login', function (req, res, next) {
  ...
  console.log(referer);
  res.render('index', { page: './login', sess: sess, referer : referer});
});//로그인 페이지 요청
